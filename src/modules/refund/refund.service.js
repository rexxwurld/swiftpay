...
async function requestRefund({
  ...
  // See payout.service.js for the rationale - mandatory in live mode only.
  if (mode === 'live' && !idempotencyKey) {
    throw new Error('idempotency_key_required_for_live_refunds');
  }
  ...
  // Fingerprint the SEMANTIC request. Computed here (after resolving the
  // transaction and refund amount) rather than from the raw caller input,
  // so a caller passing transactionId vs. reference for the same
  // underlying transaction is still recognized as the same request.
  ...
  const requestFingerprint = computeRequestFingerprint(fingerprintFields);

  // Idempotency check before claiming any refund headroom - same
  // ordering rationale as payout.service.js's requestPayout(). A retried
  ...
  if (idempotencyKey) {
    const existing = await Refund.findOne({ merchant: merchantId, idempotencyKey, mode });
    if (existing) {
      assertFingerprintMatches(existing.requestFingerprint, fingerprintFields);
      return existing;
    }
  }
  ...
  try {
    ...
    if (err.code === 11000 && idempotencyKey) {
      ...
      if (raced) {
        assertFingerprintMatches(raced.requestFingerprint, fingerprintFields);
        return raced;
      }
    }
    throw err;
