...
const ledgerEntrySchema = new mongoose.Schema(
  {
    ...
    // Test-mode and live-mode money must never be able to net together
    // into one balance - they are entirely separate universes of funds.
    // Without this, computeBalance() for a shared account (e.g. a
    // subaccount, which has no per-currency/mode wallet document of its
    // own) could sum test and live entries into one number (see audit
    // report, High #12/#13).
...
// Supports computeBalance()'s per-(account, currency, mode) aggregation -
// this is the account identity that actually matters for a balance query.
ledgerEntrySchema.index({ accountRef: 1, currency: 1, mode: 1 });
