...
const { enqueue: enqueueOutboxEvent } = require('../outbox/outbox.service');
...
async function recordIncomingPayment({
  ...
  try {
    ...
    // Enqueue post-payment side effects INSIDE this same transaction -
    // this is the actual transactional-outbox guarantee (see
    // outboxEvent.model.js). If the transaction above commits, these
    // exist; if it doesn't, they don't either. A separate, independent
    // process (src/queue/outboxWorker.js) dispatches them - this
    // function's job ends at "durably recorded that this needs to
    // happen", not at "made it happen".
    if (status === 'success' || status === 'over') {
      await enqueueOutboxEvent({
        eventType: 'deactivate_virtual_account',
        sourceType: 'transaction',
        sourceRef: transaction._id.toString(),
        payload: { merchantId: merchantId.toString(), accountNumber: virtualAccount.accountNumber },
        session,
      });

      await enqueueOutboxEvent({
        eventType: 'dispatch_merchant_webhook',
        sourceType: 'transaction',
        sourceRef: transaction._id.toString(),
        payload: { transactionId: transaction._id.toString(), merchantReference: reference },
        session,
      });
      ...
      await enqueueOutboxEvent({
        ...
        sourceType: 'transaction',
        sourceRef: transaction._id.toString(),
        payload: { transactionId: transaction._id.toString() },
        session,
      });
    }
