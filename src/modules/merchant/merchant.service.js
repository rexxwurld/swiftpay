...
async function updateWebhookUrl(merchantId, webhookUrl) {
  if (webhookUrl) {
    // Fast feedback at set-time. This is NOT the only check - DNS can
    // change before delivery, so deliverMerchantWebhook() re-validates
    // right before every send too (see src/utils/ssrfGuard.js).
