...
async function requestWithdrawal({ merchantId, amount, currency = 'NGN', idempotencyKey = null, mode }) {
  ...
  if (mode === 'live' && !idempotencyKey) {
    throw new Error('idempotency_key_required_for_live_withdrawals');
  }
  ...
  // Includes the settlement account, not just amount/currency: if the
  // merchant changes their settlement account between two calls sharing
  // an idempotency key, that's a materially different request too.
...
async function reverseWithdrawal(withdrawalId, reason) {
  ...
  try {
    ...
    await postDoubleEntry({
      ...
      amount: withdrawal.amount, currency: withdrawal.currency, mode: withdrawal.mode, sourceType: 'reversal', sourceRef: withdrawal._id.toString(),
