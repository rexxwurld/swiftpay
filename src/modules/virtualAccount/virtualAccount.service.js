...
// Internal/admin use only - deliberately not merchant-scoped. Never expose
// this directly to a merchant-authenticated route (see
// findByAccountNumberForMerchant below for that).
...
// Merchant-facing lookup. Scoped to the requesting merchant so Merchant A
// cannot fetch Merchant B's virtual account (and its populated
// merchant/customer PII) just by guessing/knowing the account number,
// which is not itself secret since it's shown on checkout pages
// (see audit report, High #14).
async function findByAccountNumberForMerchant(merchantId, accountNumber) {
  return VirtualAccount.findOne({ accountNumber, merchant: merchantId })
    .populate('bank', 'name slug')
