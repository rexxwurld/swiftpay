...
async function deliverMerchantWebhook(deliveryId) {
  ...
  try {
    // Re-validate right before sending, not just when the merchant saved
    // the URL. DNS can be repointed at a private/internal address between
    // those two moments (rebinding), so trusting an earlier check alone
    // is not enough (see audit report, High #15).
    await assertSafeWebhookUrl(delivery.webhookUrl);
    ...
    await axios.post(
      ...
      delivery.rawBody,
      {
        ...
        // Never follow redirects for a merchant-controlled destination -
        // a public URL that 302s to a private/internal address is a
        // classic SSRF bypass of a one-time hostname check.
        maxRedirects: 0,
