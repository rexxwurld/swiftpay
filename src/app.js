...
const bankRoutes = require('./modules/bankPartner/bank.routes');
...
apiV1.use('/', bankRoutes); // GET /banks, POST /bank/accounts/resolve (routes carry their own full paths)
apiV1.use('/admin', adminLimiter, adminRoutes);
// No second adminLimiter here: any /admin/settlement/* request already
// matched the /admin mount above and was counted by adminLimiter there
// first - Express runs every matching app.use() middleware regardless of
// whether the router it's paired with ends up handling the request, so
// adding the same limiter again here double-counted every settlement
// admin request against its own budget (found during a regression pass
// after adding adminLimiter in the first place).
