...
// Core logic, callable either from this CLI script (which owns its own
// mongoose connection/process lifecycle) or from the admin cron route
// (which reuses the app's existing connection and must never call
// process.exit). Do not put connect/disconnect/process.exit in here.
async function runOutboundReconciliation({ stuckAfterMinutes = STUCK_AFTER_MINUTES } = {}) {
  ...
  return { report, stillUnresolved };
}

async function main() {
  await mongoose.connect(mongoUri);

  const { report, stillUnresolved } = await runOutboundReconciliation();
...
module.exports = { runOutboundReconciliation };
