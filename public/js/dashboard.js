...
function renderTransactions() {
  ...
  // customer lookups.
  renderRecentTxV2();
  settlementPage = 0; // transactions just reloaded (e.g. mode switch) - don't leave the page pointer stranded past the end
  renderSettlementSchedule();
}
...
function settlementRows() {
  // Only transactions that actually entered the settlement pipeline -
  // a failed/flagged/pending payment never gets a settlementStatus at
  // all (see transaction.model.js), so those are correctly excluded
  // rather than showing up as a confusing blank row.
  return transactions.filter((t) => t.settlementStatus);
}

function renderSettleRow(t) {
  return `
    <tr>
      <td>${fmtDate(t.createdAt)}</td>
      <td>${esc(t.reference || t.bankReference || '—')}</td>
      <td>${money(t.amountReceived)}</td>
      <td><span class="status-pill status-${t.settlementStatus}">${esc(t.settlementStatus.replace('_', ' '))}</span></td>
      <td>${fmtDate(t.settledAt)}</td>
      <td>${fmtDate(t.availableAt)}</td>
    </tr>`;
}

function renderSettlementSchedule() {
  const rows = settlementRows();
  const tbody = document.querySelector('#settleTable tbody');
  const empty = document.getElementById('settleEmpty');
  const pagination = document.getElementById('settlePagination');
  ...
  const pageCount = Math.max(1, Math.ceil(rows.length / SETTLEMENT_PAGE_SIZE));
  if (settlementPage >= pageCount) settlementPage = pageCount - 1;
  if (settlementPage < 0) settlementPage = 0;

  const start = settlementPage * SETTLEMENT_PAGE_SIZE;
  const pageRows = rows.slice(start, start + SETTLEMENT_PAGE_SIZE);

  tbody.innerHTML = pageRows.map(renderSettleRow).join('');

  pagination.style.display = pageCount > 1 ? 'flex' : 'none';
  document.getElementById('settlePageInfo').textContent =
    `${start + 1}–${Math.min(start + SETTLEMENT_PAGE_SIZE, rows.length)} of ${rows.length}`;
  document.getElementById('settlePrevBtn').disabled = settlementPage === 0;
  document.getElementById('settleNextBtn').disabled = settlementPage >= pageCount - 1;
}

document.getElementById('settlePrevBtn')?.addEventListener('click', () => {
  settlementPage -= 1;
  renderSettlementSchedule();
});
...
document.getElementById('linkForm').addEventListener('submit', async (e) => {
  e.preventDefault();
  const amountNaira = parseFloat(document.getElementById('linkAmount').value);
  const email = document.getElementById('linkEmail').value.trim();
  const name = document.getElementById('linkName').value.trim();

  try {
    // This form collects naira from a human - /api/payments/initialize's
    // contract is minor units (kobo) like every other money-moving
    // endpoint, so convert here rather than relying on the API to do it.
...
// Auto-verify the destination account name against the bank as soon as
// bank code + account number are both filled in - never leave the
// merchant typing a name that goes completely unchecked until submit
// (see requirement: don't trust a merchant-supplied account name).
// Falls back to leaving the field free-text/editable if resolution isn't
// available right now (payout.service.js still best-effort-resolves and
// audits this same case server-side either way).
// Auto-verify a destination account name against the bank as soon as
// bank code + account number are both filled in - never leave the
// merchant typing a name that goes completely unchecked until submit
// (see requirement: don't trust a merchant-supplied account name).
// Falls back to leaving the field free-text/editable if resolution isn't
// available right now (payout.service.js/recipient.service.js still
// best-effort-resolve and audit this same case server-side either way).
// Shared by the payout form and the recipient-creation form below.
function wireAccountResolve({ bankCodeId, accountNumberId, accountNameId, statusId }) {
  const bankCodeInput = document.getElementById(bankCodeId);
  const accountNumberInput = document.getElementById(accountNumberId);
  const accountNameInput = document.getElementById(accountNameId);
  const statusEl = document.getElementById(statusId);
  if (!bankCodeInput || !accountNumberInput || !accountNameInput || !statusEl) return;
  ...
  async function tryResolve() {
    ...
    try {
      const res = await api('/api/bank/accounts/resolve', {
        method: 'POST',
        body: JSON.stringify({ bank_code: bankCode, account_number: accountNumber }),
      });
      accountNameInput.value = res.data.account_name;
      accountNameInput.readOnly = true; // provider-verified - don't let it be hand-edited back to something unverified
      statusEl.textContent = '✓ Verified with the bank';
      lastResolvedKey = key;
    } catch (err) {
      // Resolution unavailable (see bankPartner.service.js#resolveBankAccount) -
      // fall back to manual entry rather than blocking the form.
      accountNameInput.readOnly = false;
      statusEl.textContent = 'Could not auto-verify this account - you can enter the name manually, but it may need manual review before use.';
      lastResolvedKey = null;
    }
  }

  accountNumberInput.addEventListener('blur', tryResolve);
  bankCodeInput.addEventListener('blur', tryResolve);

  // If the merchant edits either field after a successful resolution,
  // the old verified name no longer applies - unlock the field and clear
  // the stale "verified" status instead of silently keeping it around.
  [bankCodeInput, accountNumberInput].forEach((el) => {
...
wireAccountResolve({ bankCodeId: 'poBankCode', accountNumberId: 'poAccountNumber', accountNameId: 'poAccountName', statusId: 'poResolveStatus' });
wireAccountResolve({ bankCodeId: 'rcpBankCode', accountNumberId: 'rcpAccountNumber', accountNameId: 'rcpAccountName', statusId: 'rcpResolveStatus' });
...
document.getElementById('payoutForm').addEventListener('submit', async (e) => {
  ...
  const recipientCode = document.getElementById('poRecipientSelect').value;

  const body = { amount: Math.round(nairaAmount * 100) };

  if (recipientCode) {
    body.recipientCode = recipientCode;
  } else {
    body.destinationBankCode = document.getElementById('poBankCode').value.trim();
    body.destinationAccountNumber = document.getElementById('poAccountNumber').value.trim();
    body.destinationAccountName = document.getElementById('poAccountName').value.trim();
  }

  try {
    await api('/api/payouts', { method: 'POST', body: JSON.stringify(body) });
    toast('Payout submitted');
    e.target.reset();
    togglePoNewDestinationFields();
...
function togglePoNewDestinationFields() {
  const select = document.getElementById('poRecipientSelect');
  const fieldsWrap = document.getElementById('poNewDestinationFields');
  const usingRecipient = !!select.value;
  fieldsWrap.style.display = usingRecipient ? 'none' : '';
  ['poBankCode', 'poAccountNumber', 'poAccountName'].forEach((id) => {
    document.getElementById(id).required = !usingRecipient;
  });
}
document.getElementById('poRecipientSelect').addEventListener('change', togglePoNewDestinationFields);
...
function verificationBadge(r) {
  if (r.verified) {
    const via = r.verificationMethod === 'admin' ? 'admin-verified' : 'bank-verified';
    return `<span class="status-pill status-successful">✓ ${esc(via)}</span>`;
  }
  return '<span class="status-pill status-pending">Needs verification</span>';
}

function renderRecipients() {
  const tbody = document.querySelector('#rcpTable tbody');
  const empty = document.getElementById('rcpEmpty');
  if (!recipients.length) {
    ...
    tbody.innerHTML = recipients.map(r => `
      <tr>
        <td>${esc(r.label)}</td>
        <td>${esc(r.accountName)}<br><span class="hint">${esc(r.accountNumber)} (${esc(r.bankCode)})</span></td>
        <td>${verificationBadge(r)}</td>
        <td><span class="status-pill status-${r.active ? 'successful' : 'failed'}">${r.active ? 'Active' : 'Deactivated'}</span></td>
        <td>${r.active ? `<button class="btn btn-sm btn-danger-outline" data-deactivate-recipient="${r._id}">Deactivate</button>` : ''}</td>
      </tr>`).join('');
  }

  // Only a verified, active recipient is payable in live mode (see
  // payout.service.js) - still list unverified ones so the merchant can
  // see they exist and need attention, but don't offer them as a
  // one-click payout destination.
  const select = document.getElementById('poRecipientSelect');
  const previousValue = select.value;
  select.innerHTML = '<option value="">New destination (enter details below)</option>' +
    recipients.filter(r => r.active && r.verified).map(r =>
      `<option value="${esc(r.recipientCode)}">${esc(r.label)} — ${esc(r.accountNumber)}</option>`
    ).join('');
  if (recipients.some(r => r.recipientCode === previousValue)) {
...
document.getElementById('recipientForm').addEventListener('submit', async (e) => {
  ...
  try {
    await api('/api/recipients', {
      method: 'POST',
      body: JSON.stringify({
        label: document.getElementById('rcpLabel').value.trim(),
        bankCode: document.getElementById('rcpBankCode').value.trim(),
        accountNumber: document.getElementById('rcpAccountNumber').value.trim(),
        accountName: document.getElementById('rcpAccountName').value.trim(),
      }),
    });
    toast('Recipient saved');
    e.target.reset();
    document.getElementById('rcpAccountName').readOnly = false;
    document.getElementById('rcpResolveStatus').textContent = '';
...
document.querySelector('#rcpTable tbody').addEventListener('click', async (e) => {
  const btn = e.target.closest('[data-deactivate-recipient]');
  if (!btn) return;
  const id = btn.getAttribute('data-deactivate-recipient');
  if (!window.confirm('Deactivate this recipient? It will no longer be usable for payouts.')) return;

  try {
    await api(`/api/recipients/${id}`, { method: 'DELETE' });
    toast('Recipient deactivated');
    await loadRecipients();
  } catch (err) {
    toast(err.message.replace(/_/g, ' '), true);
  }
